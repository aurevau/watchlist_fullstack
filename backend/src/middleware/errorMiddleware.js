...
const notFound = (req, res, next) => {
    const error = new Error(`Route ${req.originalUrl} not found`);    error.statusCode = 404; 
    next(error); 
};

/**
 * Global error handler middleware
 * handles all error in the application and send appropriate responses
...
const errorHandler = (err, req, res, next) => {
    ...
    // Handle prisma foregin key constraint violations
